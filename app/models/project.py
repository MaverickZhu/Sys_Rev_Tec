...
class Project(Base):
    ...
    # 项目分类
    project_type = Column(
        String(50), nullable=False, comment="项目类型：货物/工程/服务"
    )
    procurement_method = Column(
        String(50), comment="采购方式：公开招标/邀请招标/竞争性谈判/单一来源/询价"
    )
    ...
    status = Column(
        String(20),
        default="planning",
        comment="项目状态：planning/procurement/implementation/acceptance/completed/cancelled",
    )
    review_status = Column(
        String(20),
        default="pending",
        comment="审查状态：pending/in_progress/completed/rejected",
    )
    ...
    # 项目管理
    owner_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, comment="项目负责人"
    )
    ...
    # 风险评估
    risk_level = Column(
        String(10), default="medium", comment="风险等级：low/medium/high/critical"
    )
    ...
    #     "ProjectComparison",
    #     foreign_keys="[ProjectComparison.project_id]",
    #     back_populates="project",
    # )
    # compared_projects = relationship(
    #     "ProjectComparison",
    #     foreign_keys="[ProjectComparison.compared_project_id]",
    #     back_populates="compared_project",
    # )
...
class Issue(Base):
    ...
    severity = Column(
        String(10), default="medium", comment="严重程度：low/medium/high/critical"
    )
    ...
    status = Column(
        String(20),
        default="open",
        comment="状态：open/in_progress/resolved/closed/rejected",
    )
    priority = Column(
        String(10), default="normal", comment="优先级：low/normal/high/urgent"
    )
    ...
    # 责任人和时间
    reporter_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, comment="报告人"
    )
...
class ProjectComparison(Base):
    ...
    # compared_project = relationship("Project", back_populates="compared_projects")
