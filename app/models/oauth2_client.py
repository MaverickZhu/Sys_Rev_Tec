...
class OAuth2Client(Base):
    ...
    client_id = Column(String(255), unique=True, index=True, nullable=False, comment="客户端ID")
    ...
    # OAuth2配置
    grant_types = Column(JSON, default=["authorization_code", "refresh_token"], comment="支持的授权类型")
...
class OAuth2AuthorizationCode(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    code = Column(String(255), unique=True, index=True, nullable=False, comment="授权码")
