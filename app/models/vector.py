...
class DocumentVector(Base):
    ...
    # 元数据
    chunk_type = Column(
        String(50), default="text", comment="分块类型：text/title/table/list"
    )
...
class VectorSearchIndex(Base):
    ...
    distance_metric = Column(
        String(20), default="cosine", comment="距离度量：cosine/euclidean/inner_product"
    )
...
class SearchQuery(Base):
    ...
    query_type = Column(
        String(50), nullable=False, comment="查询类型：semantic/keyword/hybrid"
    )
...
class KnowledgeGraph(Base):
    ...
    node_type = Column(
        String(50), nullable=False, comment="节点类型：entity/concept/document"
    )
    ...
    # 实体信息
    entity_type = Column(
        String(50), comment="实体类型：person/organization/location/date"
    )
    entity_properties = Column(Text, comment="实体属性(JSON格式)")

    # 关联信息
    source_document_id = Column(
        Integer, ForeignKey("documents.id"), comment="来源文档ID"
    )
...
class KnowledgeGraphRelation(Base):
    ...
    # 关系信息
    relation_id = Column(
        String(100), unique=True, nullable=False, comment="关系唯一标识"
    )
    source_node_id = Column(
        String(100), ForeignKey("knowledge_graph_nodes.node_id"), nullable=False
    )
