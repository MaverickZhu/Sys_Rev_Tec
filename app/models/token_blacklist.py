...
class TokenBlacklist(Base):
    ...
    # Token详情
    token_type = Column(
        String(50), default="access", comment="Token类型: access, refresh"
    )
    expires_at = Column(
        DateTime(timezone=True), nullable=False, comment="Token过期时间"
    )
    ...
    blacklisted_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="加入黑名单时间"
    )
