...
class Document(Base):
    ...
    status = Column(
        String(20),
        default="uploaded",
        comment="状态：uploaded/processing/processed/error",
    )
    ...
    review_priority = Column(
        String(10),
        default="normal",
        comment="审查优先级：low/normal/high/urgent",
    )
    compliance_status = Column(
        String(20), comment="合规状态：compliant/non_compliant/pending"
    )
...
class DocumentAnnotation(Base):
    ...
    # 标注状态
    status = Column(
        String(20), default="active", comment="状态：active/resolved/deleted"
    )
...
class ComplianceCheck(Base):
    ...
    # 检查结果
    check_result = Column(
        String(20), nullable=False, comment="检查结果：pass/fail/warning/error"
    )
