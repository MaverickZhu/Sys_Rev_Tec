...
class ReviewPoint(Base):
    ...
    review_type = Column(
        String(20), nullable=False, comment="审查类型：manual/auto/both"
    )
    is_required = Column(Boolean, default=True, comment="是否必审")
    risk_level = Column(
        String(10), default="medium", comment="风险等级：low/medium/high"
    )
...
class ReviewResult(Base):
    ...
    project_review_id = Column(
        Integer, ForeignKey("project_reviews.id"), nullable=False
    )
    review_point_id = Column(Integer, ForeignKey("review_points.id"), nullable=False)

    result = Column(
        String(20), nullable=False, comment="审查结果：pass/fail/na/pending"
    )
    ...
    # 整改状态
    rectification_status = Column(
        String(20), default="pending", comment="整改状态：pending/in_progress/completed"
    )
