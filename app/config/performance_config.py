...
class PerformanceConfig:
    ...
    def from_env(cls) -> "PerformanceConfig":
        ...
        config.alert_thresholds.memory_usage_warning = float(
            os.getenv(
                "PERF_MEMORY_WARNING", config.alert_thresholds.memory_usage_warning
            )
        )
        config.alert_thresholds.memory_usage_critical = float(
            os.getenv(
                "PERF_MEMORY_CRITICAL", config.alert_thresholds.memory_usage_critical
            )
        )
        config.alert_thresholds.error_rate_warning = float(
            os.getenv(
                "PERF_ERROR_RATE_WARNING", config.alert_thresholds.error_rate_warning
            )
        )
        config.alert_thresholds.error_rate_critical = float(
            os.getenv(
                "PERF_ERROR_RATE_CRITICAL", config.alert_thresholds.error_rate_critical
            )
        )
        ...
        config.monitoring.slow_request_threshold = float(
            os.getenv(
                "PERF_SLOW_REQUEST_THRESHOLD", config.monitoring.slow_request_threshold
            )
        )
        config.monitoring.enable_alerts = (
            os.getenv("PERF_ENABLE_ALERTS", "true").lower() == "true"
        )
...
def validate_config(config: PerformanceConfig) -> List[str]:
    ...
    if (
        config.alert_thresholds.cpu_usage_warning
        >= config.alert_thresholds.cpu_usage_critical
    ):
        errors.append("CPU使用率警告阈值应小于严重阈值")

    if (
        config.alert_thresholds.memory_usage_warning
        >= config.alert_thresholds.memory_usage_critical
    ):
        errors.append("内存使用率警告阈值应小于严重阈值")

    if (
        config.alert_thresholds.error_rate_warning
        >= config.alert_thresholds.error_rate_critical
    ):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"- 响应时间告警阈值: {config.alert_thresholds.response_time_warning}s / {config.alert_thresholds.response_time_critical}s"
        )
        print(
            f"- CPU使用率告警阈值: {config.alert_thresholds.cpu_usage_warning}% / {config.alert_thresholds.cpu_usage_critical}%"
        )
        print(
            f"- 内存使用率告警阈值: {config.alert_thresholds.memory_usage_warning}% / {config.alert_thresholds.memory_usage_critical}%"
        )
        print(
            f"- 错误率告警阈值: {config.alert_thresholds.error_rate_warning}% / {config.alert_thresholds.error_rate_critical}%"
        )
        ...
        print(
            f"- 数据库监控: {'启用' if config.is_metric_enabled('database') else '禁用'}"
        )
