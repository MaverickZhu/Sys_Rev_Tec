...
class CacheMonitor:
    ...
    def _update_stats(self, operation: CacheOperation) -> None:
        ...
        self.global_stats.hit_rate = self.global_stats.hits / self.global_stats.total_requests
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for op in recent_operations:
                ...
                perf['max_response_time'] = max(perf['max_response_time'], op.response_time)
                perf['min_response_time'] = min(perf['min_response_time'], op.response_time)
            ...
            for cache_type, perf in type_performance.items():
                ...
                response_times = [
                    op.response_time for op in recent_operations 
                    if op.cache_type == cache_type
                ]
                
                if response_times:
                    perf['avg_response_time'] = sum(response_times) / len(response_times)
    ...
    def get_slow_operations(self, threshold: float = 0.1, limit: int = 50) -> List[Dict[str, Any]]:
        """获取慢操作列表
    ...
    def get_error_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            error_ops = [
                op for op in self.operations
                if op.timestamp >= cutoff_time and op.error
            ]
