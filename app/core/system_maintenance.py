...
class SystemMaintenanceManager:
    ...
    def __init__(self):
        ...
        self.log_dir = Path(settings.LOG_DIR if hasattr(settings, 'LOG_DIR') else './logs')
        self.config_dir = Path(settings.CONFIG_DIR if hasattr(settings, 'CONFIG_DIR') else './config')
    ...
    async def _backup_uploads(self, backup_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            total_size = sum(f.stat().st_size for f in uploads_backup_dir.rglob('*') if f.is_file())
