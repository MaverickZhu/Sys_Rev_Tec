...
class PermissionMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_threats(
        self, request: Request, user: User, client_ip: str, db: Session
    ) -> Optional[Response]:
        """检查威胁"""
        try:
            # 检查IP黑名单
    ...
    async def _check_brute_force(
        self, client_ip: str, user_id: int, db: Session
    ) -> bool:
        """检查暴力破解攻击"""
        # 检查最近5分钟内的失败请求数量
        recent_failures = (
    ...
    async def _check_anomaly_patterns(
        self, request: Request, user: User, client_ip: str, db: Session
    ):
        """检查异常访问模式"""
        try:
