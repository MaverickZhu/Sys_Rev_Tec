    ...
    async def load_config(self) -> CacheSystemConfig:
        ...
        try:
            ...
            if self.config_file_path.exists():
                self.current_config = load_cache_config_from_file(
                    str(self.config_file_path)
                )
                logger.info("Loaded cache config from file")
            else:
                # 使用环境默认配置
                self.current_config = get_cache_config_for_environment(
                    settings.ENVIRONMENT
                )
                logger.info(
                    f"Using default config for environment: {settings.ENVIRONMENT}"
                )
        ...
        try:
            ...
            if hit_rate < 0.7:
                recommendations["session_cache_ttl"] = min(
                    self.current_config.default_strategies.session_cache.ttl * 1.5, 3600
                )
            ...
            if avg_response_time > 100:
                recommendations["redis_pool_size"] = min(
                    self.current_config.redis_settings.pool_size * 1.2, 50
                )
    ...
    def _calculate_confidence(self, workload_stats: Dict) -> float:
        ...
        try:
            ...
            available_metrics = sum(
                1 for metric in required_metrics if metric in workload_stats
            )
