...
class ThreatDetector:
    ...
    def _load_detection_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "privilege_escalation": {
                "sensitive_actions": ["role_change", "permission_change", "user_create"],
                "level": SecurityEventLevel.HIGH
            },
            "night_activity": {
                "start_hour": 22,  # 晚上10点
    ...
    async def run_scan(self, scan_type: str = "full") -> List[Dict[str, Any]]:
        ...
        sorted_threats = sorted(unique_threats, key=lambda x: x.get("severity", 0), reverse=True)
    ...
    async def detect_brute_force_attacks(self, hours: int = 1) -> List[Dict[str, Any]]:
        ...
        ).group_by(AuditLog.ip_address, AuditLog.username).all()
        ...
        for login in failed_logins:
            if login.failed_count >= rule["threshold"]:
                ...
                # 检查是否已经存在相同的安全事件
                existing_event = self.db.query(SecurityEvent).filter(
                    and_(
    ...
    async def detect_suspicious_logins(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        ).group_by(AuditLog.user_id, AuditLog.username).all()
        ...
        for login in user_logins:
            if login.unique_ips >= rule["max_ips_per_user"]:
                ...
                existing_event = self.db.query(SecurityEvent).filter(
                    and_(
                        SecurityEvent.event_type == SecurityEventType.SUSPICIOUS_LOGIN,
                ...
                if not existing_event:
                    ...
                    event = security_event_crud.create_event(
                        ...
                        details={
                            ...
                            "time_window": hours,
                            "ip_addresses": login.ip_list if hasattr(login, 'ip_list') else []
                        }
                    )
    ...
    async def detect_privilege_escalation(self, hours: int = 24) -> List[Dict[str, Any]]:
        """检测权限提升"""
        threats = []
        start_time = datetime.utcnow() - timedelta(hours=hours)
        ...
        # 查询敏感操作
        sensitive_ops = self.db.query(AuditLog).filter(
            and_(
    ...
    async def detect_night_activities(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        # 查询深夜活动
        night_activities = self.db.query(
            AuditLog.user_id,
            AuditLog.username,
        ...
        ).group_by(AuditLog.user_id, AuditLog.username).having(
            func.count(AuditLog.id) >= 10  # 深夜活动超过10次
        ).all()
    ...
    async def detect_api_abuse(self, minutes: int = 60) -> List[Dict[str, Any]]:
        ...
        api_requests = self.db.query(
            ...
            func.count(AuditLog.id) >= rule["request_threshold"]
        ).all()
    ...
    async def detect_data_exfiltration(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        # 查询大量下载操作
        download_activities = self.db.query(
            AuditLog.user_id,
            AuditLog.username,
            AuditLog.ip_address,
            ...
            func.count(AuditLog.id) >= rule["download_threshold"]
        ).all()
    ...
    async def detect_failed_operations(self, hours: int = 1) -> List[Dict[str, Any]]:
        ...
        failed_ops = self.db.query(
            ...
            func.count(AuditLog.id) >= 10  # 失败次数阈值
        ).all()
    ...
    async def detect_account_anomalies(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        # 检测账户创建异常
        account_creations = self.db.query(
            AuditLog.user_id,
            AuditLog.username,
            ...
            func.count(AuditLog.id) >= 5  # 创建5个以上账户
        ).all()
    ...
    async def detect_injection_attempts(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        # 查询可疑请求
        suspicious_requests = self.db.query(AuditLog).filter(
            and_(
                AuditLog.created_at >= start_time,
    ...
    async def get_threat_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            "time_window": hours,
            "threat_types": {threat.event_type.value: threat.count for threat in threat_counts},
            "threat_levels": {level.level.value: level.count for level in level_counts},
            "top_source_ips": [
                {"ip": ip.source_ip, "count": ip.count}
                for ip in top_ips
            ],
