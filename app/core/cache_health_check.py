...
class CacheHealthChecker:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.redis_client = redis.Redis(
                host=self.config.redis_settings.host,
                port=self.config.redis_settings.port,
                password=self.config.redis_settings.password,
                db=self.config.redis_settings.db,
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                details={
                    ...
                    "master_last_io_seconds_ago": info.get("master_last_io_seconds_ago", "N/A")
                },
                timestamp=datetime.now(),
                response_time_ms=(time.time() - start_time) * 1000
            )
    ...
    def _generate_summary(self, checks: List[HealthCheckResult], total_time: float) -> Dict[str, Any]:
        """生成健康检查摘要"""
        summary = {
        ...
        summary["average_response_time_ms"] = round(sum(response_times) / len(response_times), 2)
        ...
        try:
            ...
            for report in recent_reports:
                for check in report.checks:
                    ...
                    component_stats[check.component]["response_times"].append(check.response_time_ms)
            ...
            for component, stats in component_stats.items():
                ...
                trends["average_response_times"][component] = round(avg_response_time, 2)
