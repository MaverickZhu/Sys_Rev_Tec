...
class AlertRule:
    ...
    def should_trigger(self, error_event: ErrorEvent, recent_count: int) -> bool:
        ...
        if severity_levels[error_event.severity] < severity_levels[self.severity_threshold]:
            return False
...
class ErrorTracker:
    ...
    def __init__(self):
        ...
        self._error_counts: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))  # 错误计数
    ...
    def _check_alert_rules(self, error_event: ErrorEvent, error_key: str) -> None:
        ...
        for rule in self._alert_rules:
            ...
            recent_errors = [
                ts for ts in self._error_counts[error_key]
                if ts >= window_start
            ]
            ...
            if rule.should_trigger(error_event, len(recent_errors)):
                ...
                try:
                    asyncio.create_task(self._send_alert(rule, error_event, len(recent_errors)))
                except RuntimeError:
                    # 不在异步环境中，使用同步方式记录报警
                     self._log_alert_sync(rule, error_event, len(recent_errors))
    
    def _log_alert_sync(self, rule: AlertRule, error_event: ErrorEvent, count: int) -> None:
        """同步方式记录报警"""
        alert_data = {
            "rule_name": rule.name,
    ...
    async def _send_alert(self, rule: AlertRule, error_event: ErrorEvent, count: int) -> None:
        """发送报警"""
        alert_data = {
            "rule_name": rule.name,
            "error_event": error_event.to_dict(),
    ...
    async def _send_email_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            msg['Subject'] = f"[ALERT] {alert_data['rule_name']} - {alert_data['error_event']['error_type']}"
    ...
    async def _send_console_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        print(f"Error: {alert_data['error_event']['error_type']} - {alert_data['error_event']['message']}")
        print(f"Count: {alert_data['count']} times in {alert_data['time_window']} seconds")
    ...
    def get_error_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats = {
                ...
                "unique_errors": len(set(self._generate_error_key(e) for e in recent_errors)),
            ...
            stats["top_errors"] = [
                {
                    "error_key": key,
                    "count": count,
                    "error_info": self._errors.get(key, {}).to_dict() if key in self._errors else {}
                }
    ...
    def clear_old_errors(self, days: int = 7) -> int:
        ...
        with self._lock:
            ...
            # 清理错误事件
            old_keys = [
                key for key, error in self._errors.items()
            ...
            logger.info(f"Cleaned {cleaned_count} old error records older than {days} days")
