...
logger = setup_logging(
    ...
    getattr(settings, 'LOG_STRUCTURED', False)
)
...
def get_structured_logger(name: Optional[str] = None, **context) -> StructuredLoggerAdapter:
    """
    获取结构化日志记录器
...
def log_database_operation(logger: logging.Logger, operation: str, table: str, 
                          duration: float, affected_rows: int = None, **extra):
    """
    记录数据库操作日志
