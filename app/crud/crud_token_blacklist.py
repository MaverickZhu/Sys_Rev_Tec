...
class CRUDTokenBlacklist(
    CRUDBase[TokenBlacklist, TokenBlacklistCreate, TokenBlacklistUpdate]
):
    """Token黑名单CRUD操作"""
    ...
    def add_to_blacklist(
        ...
        *,
        ...
        existing = (
            ...
            .filter(
                or_(TokenBlacklist.jti == jti, TokenBlacklist.token_hash == token_hash)
            )
            .first()
        )
    ...
    def get_blacklist_by_query(
        ...
        if query_params.reason:
            query = query.filter(
                TokenBlacklist.reason.ilike(f"%{query_params.reason}%")
            )
        ...
        if query_params.start_date:
            query = query.filter(
                TokenBlacklist.blacklisted_at >= query_params.start_date
            )
    ...
    def get_blacklist_stats(self, db: Session) -> TokenBlacklistStats:
        ...
        access_tokens = (
            db.query(TokenBlacklist)
            .filter(TokenBlacklist.token_type == "access")
            .count()
        )

        refresh_tokens = (
            db.query(TokenBlacklist)
            .filter(TokenBlacklist.token_type == "refresh")
            .count()
        )
        ...
        today_blacklisted = (
            db.query(TokenBlacklist)
            .filter(TokenBlacklist.blacklisted_at >= today_start)
            .count()
        )

        # 已过期
        expired_tokens = (
            db.query(TokenBlacklist).filter(TokenBlacklist.expires_at < now).count()
        )
    ...
    def remove_from_blacklist(self, db: Session, *, jti: str) -> bool:
        """从黑名单中移除token"""
        result = db.query(TokenBlacklist).filter(TokenBlacklist.jti == jti).delete()
        db.commit()
        return result > 0
