...
class CRUDAuditLog(CRUDBase[AuditLog, AuditLogCreate, Dict[str, Any]]):
    ...
    def get_statistics(
        ...
        *,
        ...
        action_stats = query.with_entities(
            self.model.action,
            func.count(self.model.id).label('count')
        ).group_by(self.model.action).order_by(desc('count')).limit(10).all()
        ...
        # 按IP统计
        ip_stats = query.filter(self.model.ip_address.isnot(None)).with_entities(
            self.model.ip_address,
            func.count(self.model.id).label('count')
        ).group_by(self.model.ip_address).order_by(desc('count')).limit(10).all()
        ...
        return {
            ...
            "top_actions": [{"action": stat.action, "count": stat.count} for stat in action_stats],
            "top_users": [
                {"user_id": stat.user_id, "username": stat.username, "count": stat.count} 
                for stat in user_stats
            ],
            "top_ips": [{"ip_address": stat.ip_address, "count": stat.count} for stat in ip_stats]
        }
    ...
    def get_user_activity(
        ...
        *,
        ...
        # 失败操作数
        failed_activities = query.filter(self.model.status.in_(["failed", "error"])).count()
        ...
        action_stats = query.with_entities(
            self.model.action,
            func.count(self.model.id).label('count')
        ).group_by(self.model.action).order_by(desc('count')).all()
    ...
    def get_security_events(
        ...
        *,
        ...
        """获取安全相关的审计日志"""
        security_actions = [
            "login_failed", "password_change", "permission_change", 
            "role_change", "account_locked", "account_unlocked",
            "password_reset", "two_factor_enabled", "two_factor_disabled"
        ]
    ...
    def detect_suspicious_activity(
        ...
        *,
        ...
        ).group_by(self.model.ip_address).having(
            func.count(self.model.id) >= 5  # 5次以上失败登录
        ).order_by(desc('failed_attempts')).all()
        ...
        ).group_by(self.model.user_id, self.model.username).having(
            func.count(func.distinct(self.model.ip_address)) >= 3  # 3个以上不同IP
        ).order_by(desc('unique_ips')).all()
        
        # 检测深夜活动
        night_activities = db.query(self.model).filter(
            and_(
                self.model.created_at >= start_time,
                or_(
