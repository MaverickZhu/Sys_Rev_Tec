...
class CRUDSecurityEvent(
    CRUDBase[SecurityEvent, SecurityEventCreate, SecurityEventUpdate]
):
    """安全事件CRUD操作类"""
    ...
    def get_statistics(
        ...
        *,
        ...
        level_stats = (
            query.with_entities(
                self.model.level, func.count(self.model.id).label("count")
            )
        ...
        type_stats = (
            query.with_entities(
                self.model.event_type, func.count(self.model.id).label("count")
            )
        ...
        # 解决状态统计
        resolved_events = query.filter(self.model.is_resolved).count()
        ...
        high_events = query.filter(self.model.level == SecurityEventLevel.HIGH).count()

        # 按源IP统计
        source_ip_stats = (
            query.filter(self.model.source_ip.isnot(None))
            .with_entities(
                self.model.source_ip, func.count(self.model.id).label("count")
            )
        ...
        resolved_query = query.filter(
            and_(self.model.is_resolved, self.model.resolved_at.isnot(None))
        )
        ...
        return {
            ...
            "level_distribution": {
                stat.level.value: stat.count for stat in level_stats
            },
            "type_distribution": {
                stat.event_type.value: stat.count for stat in type_stats
            },
            "top_source_ips": [
    ...
    def get_critical_events(
        self, db: Session, *, hours: int = 24, limit: int = 50
    ) -> List[SecurityEvent]:
        """获取严重安全事件"""
            ...
            .filter(
                and_(
                    self.model.level.in_(
                        [SecurityEventLevel.CRITICAL, SecurityEventLevel.HIGH]
                    ),
    ...
    def get_unresolved_events(
        ...
        return (
            ...
            .order_by(desc(self.model.level), desc(self.model.created_at))
            .limit(limit)
            .all()
        )
    ...
    def detect_attack_patterns(self, db: Session, *, hours: int = 24) -> Dict[str, Any]:
        """检测攻击模式"""
        start_time = datetime.utcnow() - timedelta(hours=hours)
        ...
        brute_force_attacks = (
            db.query(
                self.model.source_ip, func.count(self.model.id).label("attack_count")
            )
            .filter(
                and_(
                    self.model.event_type.in_(
                        [SecurityEventType.LOGIN_FAILED, SecurityEventType.BRUTE_FORCE]
                    ),
                    self.model.created_at >= start_time,
            ...
            .group_by(self.model.source_ip)
            .having(func.count(self.model.id) >= 5)
        ...
        injection_attacks = (
            ...
            .filter(
                and_(
                    self.model.event_type.in_(
                        [SecurityEventType.SQL_INJECTION, SecurityEventType.XSS_ATTACK]
                    ),
                    self.model.created_at >= start_time,
                )
            )
    ...
    def get_sla_breached_events(self, db: Session) -> List[SecurityEvent]:
        """获取违反SLA的事件"""
        events = db.query(self.model).filter(self.model.is_resolved == False).all()
    ...
    def create_correlation_group(
        self, db: Session, *, event_ids: List[int], correlation_id: str
    ) -> int:
        """创建事件关联组"""
        updated_count = (
            db.query(self.model)
            .filter(self.model.id.in_(event_ids))
            .update(
    ...
    def cleanup_old_events(self, db: Session, *, days: int = 365) -> int:
        ...
        deleted_count = (
            db.query(self.model)
            .filter(and_(self.model.created_at < cutoff_date, self.model.is_resolved))
            .delete()
        )
