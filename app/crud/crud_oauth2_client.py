...
from app.models.oauth2_client import OAuth2AuthorizationCode, OAuth2Client
...
class CRUDOAuth2Client(CRUDBase[OAuth2Client, OAuth2ClientCreate, OAuth2ClientUpdate]):
    ...
    def authenticate_client(
        self, db: Session, *, client_id: str, client_secret: str
    ) -> Optional[OAuth2Client]:
        """验证客户端凭证
...
class CRUDOAuth2AuthorizationCode:
    ...
    def use_authorization_code(
        self, db: Session, *, code: str, client_id: str, redirect_uri: str
    ) -> Optional[OAuth2AuthorizationCode]:
        """使用授权码
    ...
    def cleanup_expired_codes(self, db: Session) -> int:
        ...
        result = (
            db.query(OAuth2AuthorizationCode)
            .filter(OAuth2AuthorizationCode.expires_at < current_time)
            .delete()
        )
        db.commit()
        return result
