...
class EnhancedPerformanceMonitor:
    ...
    def _setup_enhanced_metrics(self):
        ...
        self.business_users_total = Gauge(
            "business_users_total", "Total number of users", registry=self.registry
        )
        ...
        self.cache_key_count = Gauge(
            "cache_key_count", "Number of cache keys", registry=self.registry
        )
    ...
    def _update_system_metrics(self, health: SystemHealth):
        ...
        # 更新网络指标
        self.system_network_bytes.labels(direction="sent").inc(
            health.network_io["bytes_sent"]
        )
        self.system_network_bytes.labels(direction="received").inc(
    ...
    async def _check_system_alerts(self, health: SystemHealth):
        ...
        if health.memory_usage >= self._alert_thresholds["memory_usage_critical"]:
            ...
            self.performance_alerts.labels(
                alert_type="memory_usage", severity="critical"
            ).inc()
            ...
            self.performance_alerts.labels(
                alert_type="memory_usage", severity="warning"
            ).inc()
    ...
    def record_cache_performance(
        self, memory_usage: int, key_count: int, eviction_reason: str = None
    ):
        """记录缓存性能指标"""
    ...
    def _generate_optimization_recommendations(
        self, system_health: SystemHealth, business_metrics: BusinessMetrics
    ) -> List[str]:
        """生成优化建议"""
        recommendations = []
