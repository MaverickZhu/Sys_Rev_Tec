...
from app.services.cache_monitor import CacheMetrics, get_cache_monitor
...
class CacheOptimizer:
    ...
    def __init__(self):
        ...
        self.optimization_interval = 300  # 默认5分钟
        self.auto_optimization = True  # 默认启用自动优化
    ...
    async def configure(self, config: Dict[str, Any]) -> None:
        ...
        """
        try:
            logger.info("Configuring cache optimizer...")

            # 更新优化间隔
            if "optimization_interval" in config:
                self.optimization_interval = config["optimization_interval"]
                logger.info(
                    f"Optimization interval set to {self.optimization_interval} seconds"
                )
            ...
            if "auto_optimization" in config:
                ...
                logger.info(
                    f"Auto optimization {'enabled' if self.auto_optimization else 'disabled'}"
                )

            # 更新优化阈值
            if "optimization_thresholds" in config:
                thresholds = config["optimization_thresholds"]
                if "hit_rate_threshold" in thresholds:
                    self.optimization_thresholds["low_hit_rate"] = (
                        thresholds["hit_rate_threshold"] * 100
                    )
                if "memory_threshold" in thresholds:
                    self.optimization_thresholds["high_memory_usage"] = int(
                        thresholds["memory_threshold"] * 1024 * 1024 * 1024
                    )
                if "response_time_threshold" in thresholds:
                    self.optimization_thresholds["high_response_time"] = thresholds[
                        "response_time_threshold"
                    ]
                logger.info(
                    f"Optimization thresholds updated: {self.optimization_thresholds}"
                )

            logger.info("Cache optimizer configuration completed")

        except Exception as e:
            logger.error(f"Failed to configure cache optimizer: {e}")
            raise
    ...
    async def start_background_optimization(self):
        ...
        try:
            if self.auto_optimization:
                await self.start_scheduler(self.optimization_interval)
                logger.info("Background optimization started")
            else:
                logger.info(
                    "Auto optimization is disabled, skipping background optimization"
                )
        except Exception as e:
    ...
    def _analyze_performance_issues(
        ...
        if (
            ...
            > self.optimization_thresholds["high_response_time"]
        ):
            issues.append(
                (
    ...
    def _find_running_task(
        self, cache_type: str, optimization_type: OptimizationType
        ...
        for task in self.tasks.values():
            if (
                task.cache_type == cache_type
                and task.optimization_type == optimization_type
                and task.status == OptimizationTaskStatus.RUNNING
            ):
                return task
        return None
    ...
    async def _optimize_ttl(
        self, cache_type: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        优化TTL设置
        """
    ...
    async def _preload_cache(
        ...
        try:
            ...
            # 示例：预加载一些测试数据
            test_data = {f"preload_key_{i}": f"preload_value_{i}" for i in range(10)}

            success = await cache_service.warmup_cache(cache_type, test_data)

            return {"preloaded_keys": len(test_data), "success": success}
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        completed_tasks = len(
            [
                ...
                if t.status == OptimizationTaskStatus.COMPLETED
            ]
        )
        failed_tasks = len(
            [
                t
                for t in self.tasks.values()
                if t.status == OptimizationTaskStatus.FAILED
            ]
        )
        running_tasks = len(
            [
                t
                for t in self.tasks.values()
                if t.status == OptimizationTaskStatus.RUNNING
            ]
        )
