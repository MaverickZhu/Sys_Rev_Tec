...
class CacheSystemInitializer:
    ...
    async def _initialize_strategy_manager(self, config: Optional[Dict[str, Any]]) -> None:
        """初始化缓存策略管理器"""
        try:
            ...
            if config and "strategies" in config:
                default_strategies.update(config["strategies"])
            
            # 批量创建策略
            for strategy_name, strategy_config in default_strategies.items():
                await cache_strategy_manager.create_strategy(
                    strategy_name, strategy_config
                )
            
            self.components_status["cache_strategy"] = True
            logger.info(f"Cache strategy manager initialized with {len(default_strategies)} strategies")
    ...
    async def _start_background_tasks(self) -> None:
        ...
        try:
            ...
            # 启动健康检查任务
            asyncio.create_task(cache_manager.start_health_check_loop())
