...
from app.services.multi_level_cache import get_multi_cache_service, CacheHitType
...
class CacheMonitor:
    ...
    async def start_monitoring(self):
        ...
        logger.info(f"Cache monitoring started with {self.collection_interval}s interval")
    ...
    async def _collect_metrics(self):
        ...
        try:
            ...
            # 错误率
            error_rate = self.hit_miss_stats['errors'] / total_requests if total_requests > 0 else 0
            ...
            metrics = [
                ...
                CacheMetric('avg_response_time', MetricType.GAUGE, avg_response_time, now),
                ...
                CacheMetric('active_connections', MetricType.GAUGE, active_connections, now)
            ]
    ...
    def get_current_metrics(self, cache_type: Optional[str] = None) -> Optional[CacheMetrics]:
        """获取当前缓存指标"""
        with self._lock:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            'current_status': {
                ...
                'total_requests': latest_snapshot.total_requests if latest_snapshot else 0,
            ...
            'health_score': self._calculate_health_score(latest_snapshot, active_alerts)
        }
    
    def _calculate_health_score(self, snapshot: Optional[CachePerformanceSnapshot], 
                               alerts: List[CacheAlert]) -> float:
        """计算健康评分 (0-100)"""
        if not snapshot:
            return 0.0
