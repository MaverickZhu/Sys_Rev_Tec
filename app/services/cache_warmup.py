...
class CacheWarmupService:
    ...
    async def warmup_all(self, priority_filter: Optional[WarmupPriority] = None) -> List[WarmupResult]:
        """执行所有预热任务"""
        ...
        if priority_filter:
            tasks_to_run = [task for task in tasks_to_run if task.priority == priority_filter]
    ...
    async def _execute_warmup_task(self, task: WarmupTask) -> WarmupResult:
        ...
        try:
            ...
            logger.info(f"Warmup task '{task.name}' completed: {items_loaded} items in {duration:.2f}s")
    ...
    async def _load_system_config(self) -> List[Dict[str, Any]]:
        ...
        # 模拟系统配置数据
        configs = [
            {"key": "app_name", "value": getattr(settings, 'PROJECT_NAME', 'System Review')},
            ...
            {"key": "ai_service_url", "value": getattr(settings, 'AI_SERVICE_URL', 'http://localhost:8001')},
            {"key": "redis_url", "value": getattr(settings, 'REDIS_URL', 'redis://localhost:6379')}
        ]
        return configs
