...
class SecurityService:
    ...
    async def get_security_metrics(self) -> SecurityMetrics:
        ...
        # 今日威胁数量
        today_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            SecurityEvent.created_at >= today_start
        ).scalar() or 0
        
        # 本周威胁数量
        week_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            SecurityEvent.created_at >= week_start
        ).scalar() or 0
        
        # 活跃威胁数量（未解决）
        active_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            SecurityEvent.is_resolved == False
        ).scalar() or 0
        
        # 高危威胁数量
        critical_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            and_(
        ...
        # 今日登录失败次数
        today_failed_logins = self.db.query(func.count(AuditLog.id)).filter(
            and_(
        ...
        # 今日活跃用户数
        today_active_users = self.db.query(func.count(func.distinct(AuditLog.user_id))).filter(
            and_(
                AuditLog.created_at >= today_start,
        ...
        failed_requests = self.db.query(func.count(AuditLog.id)).filter(
            and_(
                AuditLog.created_at >= today_start,
        ...
        system_availability = ((total_requests - failed_requests) / total_requests) * 100
        
        # 威胁趋势（与上周比较）
        prev_week_start = week_start - timedelta(days=7)
        prev_week_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            and_(
    ...
    async def get_security_alerts(self, limit: int = 10) -> List[SecurityAlert]:
        ...
        events = self.db.query(SecurityEvent).filter(
            ...
            desc(SecurityEvent.severity_score),
    ...
    async def get_security_trends(self, days: int = 30) -> List[SecurityTrend]:
        ...
        # 按日期统计威胁数量
        daily_threats = self.db.query(
            func.date(SecurityEvent.created_at).label('date'),
            func.count(SecurityEvent.id).label('count')
        ).filter(
            func.date(SecurityEvent.created_at) >= start_date
        ).group_by(
            func.date(SecurityEvent.created_at)
        ).all()
        
        # 按日期统计失败登录
        daily_failed_logins = self.db.query(
    ...
    async def get_user_activity_summary(self, hours: int = 24) -> List[UserActivitySummary]:
        """获取用户活动摘要"""
        ...
        user_activities = self.db.query(
            ...
            func.sum(
                func.case(
                    [(AuditLog.status.in_(["failed", "error"]), 1)],
                    else_=0
                )
    ...
    async def get_system_health_status(self) -> SystemHealthStatus:
        ...
        # 系统负载（基于请求数量）
        recent_requests = self.db.query(func.count(AuditLog.id)).filter(
            AuditLog.created_at >= hour_ago
        ).scalar() or 0
        
        # 错误率
        recent_errors = self.db.query(func.count(AuditLog.id)).filter(
            and_(
                AuditLog.created_at >= hour_ago,
                AuditLog.status.in_(["failed", "error"])
            )
        ...
        # 活跃连接数（基于唯一IP）
        active_connections = self.db.query(
    ...
    async def get_threat_detection_results(self) -> ThreatDetectionResult:
        ...
        # 获取威胁摘要
        threat_summary = await self.threat_detector.get_threat_summary(24)
        ...
        return ThreatDetectionResult(
            ...
            high_priority_threats=len([t for t in threats if t.get("severity", 0) >= 70]),
    ...
    async def get_security_recommendations(self) -> List[SecurityRecommendation]:
        ...
        # 检查长时间未更新密码的用户
        old_password_users = self.db.query(func.count(User.id)).filter(
            or_(
        ...
        # 检查权限过度分配
        superusers = self.db.query(func.count(User.id)).filter(
    ...
    async def get_security_statistics(self, days: int = 30) -> SecurityStatistics:
        ...
        # 威胁统计
        total_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            SecurityEvent.created_at >= start_date
        ).scalar() or 0
        
        resolved_threats = self.db.query(func.count(SecurityEvent.id)).filter(
            and_(
                SecurityEvent.created_at >= start_date,
        ...
        # 威胁级别分布
        threat_levels = self.db.query(
            SecurityEvent.level,
            func.count(SecurityEvent.id).label('count')
        ).filter(
            SecurityEvent.created_at >= start_date
        ).group_by(SecurityEvent.level).all()
        
        # 用户活动统计
        total_logins = self.db.query(func.count(AuditLog.id)).filter(
            and_(
                AuditLog.action == "login",
        ...
        return SecurityStatistics(
            ...
            threat_resolution_rate=round((resolved_threats / max(total_threats, 1)) * 100, 2),
            threat_types_distribution={threat.event_type.value: threat.count for threat in threat_types},
            threat_levels_distribution={level.level.value: level.count for level in threat_levels},
            total_login_attempts=total_logins,
            failed_login_attempts=failed_logins,
            login_success_rate=round(((total_logins - failed_logins) / max(total_logins, 1)) * 100, 2),
    ...
    async def resolve_security_event(self, event_id: int, resolved_by: int, resolution_notes: str = None) -> SecurityEvent:
        """解决安全事件"""
    ...
    async def run_threat_detection(self, scan_type: str = "quick") -> List[Dict[str, Any]]:
        """运行威胁检测"""
