...
async def get_database_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        stats["connection_count"] = (
            connection_result.total_connections if connection_result else 0
        )
        stats["active_queries"] = (
            connection_result.active_connections if connection_result else 0
        )

        # 获取慢查询统计
        slow_query_result = db.execute(
            text(
                """
        ...
        stats["slow_queries_count"] = (
            slow_query_result.slow_queries if slow_query_result else 0
        )
        ...
        stats["cache_hit_ratio"] = (
            float(cache_result.cache_hit_ratio)
            if cache_result and cache_result.cache_hit_ratio
            else 0.0
        )
        ...
        stats["avg_query_time"] = (
            float(avg_time_result.avg_query_time)
            if avg_time_result and avg_time_result.avg_query_time
            else 0.0
        )
...
async def analyze_query(
    request: QueryOptimizationRequest, current_user: User = Depends(get_current_user)
) -> QueryAnalysisResult:
    """
    分析SQL查询的性能并提供优化建议
    """
    ...
    try:
        ...
        # 验证查询语句
        if not query or query.lower().startswith(
    ...
    try:
        ...
        queries_data = [
            {
                ...
                "stddev_time_ms": (
                    float(row.stddev_exec_time) if row.stddev_exec_time else 0
                ),
                "total_rows": row.rows,
        ...
        return {
            ...
            "data": {
                ...
                "filter": {"min_duration_ms": min_duration, "limit": limit},
            },
            "timestamp": datetime.now().isoformat(),
        }
    ...
    try:
        ...
        for operation in operations:
            try:
                ...
                results.append(
                    {"operation": operation, "status": "error", "message": str(e)}
                )
        ...
        # 记录操作日志
        logger.info(
            f"用户 {current_user.username} 对表 {table_name} 执行了优化操作: {operations}"
        )
