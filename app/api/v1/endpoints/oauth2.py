...
def get_oauth2_client(
    *,
    ...
    if not client:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="客户端不存在"
        )
...
def authorize_get(
    ...
    if not client or not client.is_active:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="无效的客户端"
        )
    ...
    for s in scopes:
        if s in oauth2_server.get_supported_scopes():
            scope_descriptions.append(
                {"name": s, "description": oauth2_server.get_supported_scopes()[s]}
            )
...
def authorize_post(
    ...
    if action == "deny":
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="无效的操作"
        )
