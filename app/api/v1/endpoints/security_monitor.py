...
async def get_audit_logs(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取审计日志失败"
        )
...
async def get_security_events(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取安全事件失败"
        )
...
async def resolve_security_event(
    *,
    ...
    try:
        ...
        if not event:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="安全事件不存在"
            )

        if event.is_resolved:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="安全事件已经解决"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="解决安全事件失败"
        )
    ...
    *,
    ...
    try:
        ...
        background_tasks.add_task(
            _run_threat_detection_task, db, current_user.id, scan_type
        )
...
async def export_audit_logs(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="导出审计日志失败"
        )
...
async def _get_security_metrics(
    ...
    total_audit_logs = (
        db.query(AuditLog)
        .filter(AuditLog.created_at.between(start_time, end_time))
        .count()
    )
    ...
    security_events = (
        ...
        .filter(SecurityEvent.created_at.between(start_time, end_time))
        .count()
    )
...
async def _get_security_alerts(
    ...
    events = (
        ...
        .filter(
            and_(
                SecurityEvent.created_at.between(start_time, end_time),
                SecurityEvent.level.in_(
                    [SecurityEventLevel.HIGH, SecurityEventLevel.CRITICAL]
                ),
...
async def search_audit_logs(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="搜索审计日志失败"
        )
...
async def get_user_operation_history(
    *,
    ...
    try:
        ...
        # 获取数据
        logs = audit_log_crud.search(db, user_id=user_id, skip=skip, limit=limit)
...
async def get_failed_operations(
    *,
    ...
    try:
        # 获取总数
        total = audit_log_crud.count(
            db, status="FAILED", start_time=start_time, end_time=end_time
        )
...
async def get_high_risk_operations(
    *,
    ...
    try:
        ...
        # 构建查询
        query = db.query(AuditLog).filter(AuditLog.action.in_(high_risk_actions))
...
async def _get_security_trends(
    ...
    while current_time < end_time:
        ...
        events_count = (
            db.query(SecurityEvent)
            .filter(SecurityEvent.created_at.between(current_time, next_time))
            .count()
        )
        ...
        trends.append(
            SecurityTrend(
                ...
                failed_logins=failed_logins,
                threat_score=min(
                    100, (events_count * 10 + failed_logins * 5)
                ),  # 简单的威胁评分
            )
        )
...
async def _get_threat_detection_results(
    db: Session, start_time: datetime, end_time: datetime
) -> ThreatDetectionResult:
    """获取威胁检测结果"""
    # 这里可以集成实际的威胁检测逻辑
