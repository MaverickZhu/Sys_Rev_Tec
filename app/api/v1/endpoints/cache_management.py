...
async def clear_cache(
    cache_type: Optional[str] = Query(None, description="缓存类型，None表示清空所有缓存"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> ResponseModel:
    """清空缓存
    ...
    try:
        ...
        if cache_type:
            ...
            if cache_type == "user_permissions":
                ...
                cache_manager.clear_role_cache()
                message = f"已清空 {cache_type} 缓存"
            elif cache_type == "resource_permissions":
                cache_manager.clear_resource_cache()
                message = f"已清空 {cache_type} 缓存"
            else:
                raise HTTPException(
...
async def optimize_config_for_workload(
    ...
    try:
        config_manager = get_cache_config_manager()
        optimization_result = await config_manager.optimize_config_for_workload(workload_stats)
