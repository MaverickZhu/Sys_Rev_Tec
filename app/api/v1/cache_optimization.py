...
from app.tasks.cache_optimization import OptimizationTask, cache_optimization_scheduler
...
async def get_cache_strategies(
    ...
    try:
        ...
        return {"success": True, "data": strategies, "message": "获取缓存策略成功"}
...
async def get_cache_strategy_detail(
    cache_type: str, current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    获取指定类型的缓存策略详情
    """
...
async def update_cache_strategy(
    ...
    try:
        ...
        # 记录操作日志
        logger.info(
            f"用户 {current_user.username} 更新了缓存策略 {cache_type}: {strategy_update}"
        )
...
async def get_cache_stats(
    cache_type: Optional[str] = Query(
        None, description="缓存类型，不指定则返回所有类型"
    ),
    current_user: User = Depends(get_current_user),
) -> Dict[str, Any]:
    """
...
async def cleanup_cache(
    ...
    try:
        ...
        # 记录操作日志
        logger.info(
            f"用户 {current_user.username} 清理了缓存 {cache_type}, 模式: {cleanup_pattern}, 清理数量: {cleared_count}"
        )
...
async def get_task_detail(
    ...
    try:
        ...
        return {"success": True, "data": task_status, "message": "获取任务详情成功"}
    except HTTPException:
        raise
    except Exception as e:
...
async def _get_cache_type_stats(cache_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "last_access_time": cache_metrics.get(
