...
def create_default_review_stages(db: Session):
    ...
    for stage_data in stages_data:
        existing_stage = db.query(ReviewStage).filter(
...
def create_default_review_points(db: Session):
    ...
    for point_data in points_data:
        if point_data["stage_id"]:
            existing_point = db.query(ReviewPoint).filter(
                ReviewPoint.code == point_data["code"]
            ).first()
    ...
    rules_data = [
        {
            ...
            "check_pattern": "budget_amount <= annual_budget * 0.3",
            "severity": "high",
        ...
        },
        {
            ...
            "check_pattern": "if budget_amount >= public_tender_threshold then method = 'public_tender'",
            "severity": "critical",
