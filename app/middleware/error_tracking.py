...
class ErrorTrackingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, track_all_errors: bool = True, track_4xx_errors: bool = False
    ):
        super().__init__(app)
    ...
    async def _track_http_error(
        self, request: Request, response: Response, client_ip: str, request_id: str
    ):
        """追踪HTTP错误"""
        severity = (
        ...
        track_error(
            ...
            message=f"HTTP {response.status_code} error on"
    ...
    async def _track_api_exception(
        self,
        request: Request,
        ...
        track_error(
            ...
            details={
                ...
                "query_params": dict(request.query_params),
                "exception_details": (
                    exception.details if hasattr(exception, "details") else None
                ),
            },
        )

    async def _track_unhandled_exception(
        self, request: Request, exception: Exception, client_ip: str, request_id: str
    ):
        """追踪未处理的异常"""
        track_error(
