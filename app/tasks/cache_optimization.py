...
from app.config.cache_strategy import cache_strategy_manager, CacheStrategy
...
class CacheOptimizationScheduler:
    ...
    async def _schedule_analysis_tasks(self, current_time: datetime):
        ...
        for cache_type in cache_strategy_manager.strategies.keys():
            last_analysis = self.last_optimization_time.get(f"{cache_type}_analysis", 
                                                           current_time - timedelta(hours=1))
            
            if (current_time - last_analysis).total_seconds() >= self.config["analysis_interval"]:
    ...
    async def _schedule_optimization_tasks(self, current_time: datetime):
        """调度优化任务"""
        for cache_type in cache_strategy_manager.strategies.keys():
            last_optimization = self.last_optimization_time.get(f"{cache_type}_optimization",
                                                               current_time - timedelta(hours=2))
            
            if (current_time - last_optimization).total_seconds() >= self.config["optimization_interval"]:
                # 检查是否需要优化
                if await self._should_optimize(cache_type):
                    ...
                    self.tasks.append(task)
                    self.last_optimization_time[f"{cache_type}_optimization"] = current_time
    
    async def _schedule_cleanup_tasks(self, current_time: datetime):
        """调度清理任务"""
        last_cleanup = self.last_optimization_time.get("global_cleanup",
                                                      current_time - timedelta(hours=2))
    ...
    async def _schedule_report_tasks(self, current_time: datetime):
        """调度报告任务"""
        last_report = self.last_optimization_time.get("daily_report",
                                                     current_time - timedelta(days=2))
        
        if (current_time - last_report).total_seconds() >= self.config["report_interval"]:
            task = OptimizationTask(
    ...
    async def _should_optimize(self, cache_type: str) -> bool:
        ...
        try:
            ...
            return (
                hit_rate < threshold["hit_rate_low"] or
                hit_rate > threshold["hit_rate_high"] or
                memory_usage_ratio > threshold["memory_usage_high"] or
                avg_response_time > threshold["response_time_high"]
            )
        except Exception as e:
    ...
    async def _execute_tasks(self):
        ...
        while self.running:
            try:
                ...
                available_slots = self.config["max_concurrent_tasks"] - len(running_tasks)
    ...
    async def _execute_single_task(self, task: OptimizationTask):
        ...
        try:
            ...
            self.logger.info(f"任务执行完成: {task.task_id}, 耗时: {task.execution_time:.2f}秒")
    ...
    async def _execute_cleanup_task(self, task: OptimizationTask) -> Dict[str, Any]:
        ...
        if task.cache_type == "all":
            ...
            # 单个缓存类型清理
            cleanup_results[task.cache_type] = await self._cleanup_cache_type(task.cache_type)
    ...
    async def _get_cache_stats(self, cache_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "memory_usage_ratio": cache_info.get("memory_usage_mb", 0) / 1024,  # 假设1GB限制
    ...
    async def _apply_optimization(self, cache_type: str, recommendation: Dict[str, Any]) -> bool:
        """应用优化建议"""
        try:
            ...
            if action == "increase_ttl":
                ...
                if strategy:
                    ...
                    return cache_strategy_manager.update_strategy(cache_type, ttl=new_ttl)
                ...
                return cache_strategy_manager.update_strategy(cache_type, compression=True)
            
            elif action == "upgrade_to_l1":
                from app.config.cache_strategy import CacheLevel
                return cache_strategy_manager.update_strategy(cache_type, level=CacheLevel.L1_MEMORY)
    ...
    async def _generate_performance_report(self) -> Dict[str, Any]:
        ...
        if cache_count > 0:
            report["summary"] = {
                ...
                "optimization_count": len([h for h in self.optimization_history 
                                          if h.get("action") == "optimization"]),
                "last_optimization": max([h.get("timestamp", "") for h in self.optimization_history] + [""])
            }
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "total_optimizations": len([h for h in recent_history if h.get("action") == "optimization"]),
            "total_analyses": len([h for h in recent_history if h.get("action") == "analysis"]),
