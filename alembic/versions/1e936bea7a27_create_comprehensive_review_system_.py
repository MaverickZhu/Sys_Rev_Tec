"""Create comprehensive review system models

Revision ID: 1e936bea7a27
Revises: 26769560901e
Create Date: 2025-07-23 13:11:01.679098
...
revision: str = '1e936bea7a27'
down_revision: Union[str, Sequence[str], None] = '26769560901e'
...
def upgrade() -> None:
    ...
    op.add_column('projects', sa.Column('project_code', sa.String(length=50), nullable=False, comment='项目编号'))
    op.add_column('projects', sa.Column('project_type', sa.String(length=50), nullable=False, comment='项目类型：货物/工程/服务'))
    op.add_column('projects', sa.Column('procurement_method', sa.String(length=50), nullable=True, comment='采购方式：公开招标/邀请招标/竞争性谈判/单一来源/询价'))
    op.add_column('projects', sa.Column('budget_source', sa.String(length=100), nullable=True, comment='资金来源'))
    op.add_column('projects', sa.Column('budget_amount', sa.Numeric(precision=15, scale=2), nullable=True, comment='预算金额'))
    op.add_column('projects', sa.Column('contract_amount', sa.Numeric(precision=15, scale=2), nullable=True, comment='合同金额'))
    op.add_column('projects', sa.Column('final_amount', sa.Numeric(precision=15, scale=2), nullable=True, comment='最终金额'))
    op.add_column('projects', sa.Column('planned_start_date', sa.Date(), nullable=True, comment='计划开始日期'))
    op.add_column('projects', sa.Column('planned_end_date', sa.Date(), nullable=True, comment='计划结束日期'))
    op.add_column('projects', sa.Column('actual_start_date', sa.Date(), nullable=True, comment='实际开始日期'))
    op.add_column('projects', sa.Column('actual_end_date', sa.Date(), nullable=True, comment='实际结束日期'))
    op.add_column('projects', sa.Column('status', sa.String(length=20), nullable=True, comment='项目状态：planning/procurement/implementation/acceptance/completed/cancelled'))
    op.add_column('projects', sa.Column('review_status', sa.String(length=20), nullable=True, comment='审查状态：pending/in_progress/completed/rejected'))
    op.add_column('projects', sa.Column('procuring_entity', sa.String(length=200), nullable=True, comment='采购人'))
    op.add_column('projects', sa.Column('procurement_agency', sa.String(length=200), nullable=True, comment='采购代理机构'))
    op.add_column('projects', sa.Column('winning_bidder', sa.String(length=200), nullable=True, comment='中标人'))
    op.add_column('projects', sa.Column('department', sa.String(length=100), nullable=True, comment='所属部门'))
    op.add_column('projects', sa.Column('priority', sa.String(length=10), nullable=True, comment='优先级：low/normal/high/urgent'))
    op.add_column('projects', sa.Column('review_required', sa.Boolean(), nullable=True, comment='是否需要审查'))
    op.add_column('projects', sa.Column('current_review_stage', sa.String(length=20), nullable=True, comment='当前审查阶段'))
    op.add_column('projects', sa.Column('review_progress', sa.Integer(), nullable=True, comment='审查进度(百分比)'))
    op.add_column('projects', sa.Column('risk_level', sa.String(length=10), nullable=True, comment='风险等级：low/medium/high/critical'))
    op.add_column('projects', sa.Column('risk_factors', sa.Text(), nullable=True, comment='风险因素'))
    op.add_column('projects', sa.Column('notes', sa.Text(), nullable=True, comment='备注'))
    op.add_column('projects', sa.Column('tags', sa.Text(), nullable=True, comment='标签(JSON格式)'))
    op.add_column('projects', sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True))
    op.add_column('projects', sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True))
    # SQLite doesn't support ALTER COLUMN TYPE, skipping these operations
    # The new columns will be created with correct types
    ...
    op.add_column('users', sa.Column('full_name', sa.String(length=100), nullable=True, comment='姓名'))
    op.add_column('users', sa.Column('employee_id', sa.String(length=50), nullable=True, comment='工号'))
    op.add_column('users', sa.Column('department', sa.String(length=100), nullable=True, comment='部门'))
    op.add_column('users', sa.Column('position', sa.String(length=100), nullable=True, comment='职位'))
    op.add_column('users', sa.Column('phone', sa.String(length=20), nullable=True, comment='电话'))
    op.add_column('users', sa.Column('is_verified', sa.Boolean(), nullable=True, comment='是否已验证'))
    op.add_column('users', sa.Column('role_id', sa.Integer(), nullable=True, comment='角色ID'))
    op.add_column('users', sa.Column('permissions', sa.Text(), nullable=True, comment='额外权限(JSON格式)'))
    op.add_column('users', sa.Column('reviewer_level', sa.String(length=20), nullable=True, comment='审查员级别：junior/senior/expert/chief'))
    op.add_column('users', sa.Column('specialties', sa.Text(), nullable=True, comment='专业领域(JSON格式)'))
    op.add_column('users', sa.Column('certification', sa.String(length=100), nullable=True, comment='资质证书'))
    op.add_column('users', sa.Column('last_login', sa.DateTime(timezone=True), nullable=True, comment='最后登录时间'))
    op.add_column('users', sa.Column('login_count', sa.Integer(), nullable=True, comment='登录次数'))
    op.add_column('users', sa.Column('failed_login_attempts', sa.Integer(), nullable=True, comment='失败登录次数'))
    ...
    op.add_column('users', sa.Column('password_reset_token', sa.String(length=255), nullable=True, comment='密码重置令牌'))
    op.add_column('users', sa.Column('password_reset_expires', sa.DateTime(timezone=True), nullable=True, comment='重置令牌过期时间'))
    ...
    # SQLite constraint operations moved to table creation
...
def downgrade() -> None:
    ...
    op.drop_column('projects', 'review_progress')
    op.drop_column('projects', 'current_review_stage')
    op.drop_column('projects', 'review_required')
    op.drop_column('projects', 'priority')
    op.drop_column('projects', 'department')
    op.drop_column('projects', 'winning_bidder')
    op.drop_column('projects', 'procurement_agency')
    op.drop_column('projects', 'procuring_entity')
    ...
    op.drop_column('projects', 'final_amount')
