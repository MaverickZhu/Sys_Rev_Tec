...
class VectorDatabase:
    ...
    async def batch_insert_document_vectors(
        ...
        async with self.get_connection() as conn:
            ...
            # 执行批量插入
            results = await conn.fetch(query, *batch_data[0])
            ids = []
    ...
    async def hybrid_search(
        ...
        query_parts.extend([
            ...
            f"       ({alpha} * COALESCE(v.vector_similarity, 0) + {1-alpha} * COALESCE(t.text_rank, 0)) as hybrid_score",
    ...
    async def get_document_vectors(
        ...
        """获取文档的所有向量"""
        query = """
            SELECT id, document_id, content, metadata, chunk_index, created_at, updated_at
            FROM document_vectors
            WHERE document_id = $1
    ...
    async def log_search_history(
        ...
        query = """
            INSERT INTO vector_search_history (
                query_text, search_type, results_count, execution_time_ms, user_id, metadata
            ) VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id
        """
    ...
    async def update_vector_stats(
        ...
        """更新向量统计"""
        query = """
            INSERT INTO vector_index_stats (operation_type, operation_count, total_execution_time_ms)
            VALUES ($1, $2, $3)
            ON CONFLICT (operation_type) DO UPDATE SET
                operation_count = vector_index_stats.operation_count + $2,
                total_execution_time_ms = vector_index_stats.total_execution_time_ms + $3,
                updated_at = NOW()
        """
    ...
    async def get_vector_stats(self) -> List[Dict[str, Any]]:
        """获取向量统计"""
        query = """
            SELECT operation_type, operation_count, total_execution_time_ms,
        ...
        try:
            async with self.get_connection() as conn:
                ...
                table_stats = await conn.fetch("""
                    ...
                    WHERE tablename IN ('document_vectors', 'vector_search_history', 'vector_index_stats')
                """)
