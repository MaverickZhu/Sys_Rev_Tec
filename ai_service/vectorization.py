...
from ai_service.utils.text_processing import get_text_processor, TextChunk
...
class VectorizationService:
    ...
    def __init__(self):
        ...
        self.stats = {
            ...
            "provider_stats": {provider.value: {"requests": 0, "total_time": 0.0} for provider in EmbeddingProvider}
        }
    ...
    async def _initialize_ollama(self):
        ...
        try:
            ...
            if settings.OLLAMA_EMBEDDING_MODEL not in available_models:
                ...
                logger.info(f"✅ Ollama嵌入模型 {settings.OLLAMA_EMBEDDING_MODEL} 已就绪")
        ...
        **kwargs
        ...
        try:
            if provider == EmbeddingProvider.OLLAMA:
                embedding = await self._get_ollama_embedding(processed_text, model, **kwargs)
            elif provider == EmbeddingProvider.AZURE_OPENAI:
                embedding = await self._get_azure_embedding(processed_text, model, **kwargs)
            else:
            ...
            self.stats["provider_stats"][provider.value]["total_time"] += processing_time
    ...
    async def _get_ollama_embedding(self, text: str, model: str, **kwargs) -> List[float]:
        """使用Ollama获取嵌入向量"""
    ...
    async def _get_azure_embedding(self, text: str, model: str, **kwargs) -> List[float]:
    ...
    async def get_batch_embeddings(
        ...
        **kwargs
        ...
        for i in range(0, len(texts), batch_size):
            ...
            tasks = [
                self.get_embedding(
                    text, provider, model, use_cache, **kwargs
                )
    ...
    async def vectorize_document(
        ...
        **kwargs
        ...
        try:
            ...
            # 构建结果
            vectorized_chunks = []
            for i, (chunk, embedding) in enumerate(zip(chunks, batch_result.embeddings)):
            ...
            structured_logger.log_vectorization(
                ...
                model=batch_result.model,
                provider=batch_result.provider,
                dimension=len(batch_result.embeddings[0]) if batch_result.embeddings and batch_result.embeddings[0] else 0,
                processing_time=total_processing_time,
                cache_hit=False,
                extra_data={
                    "document_id": document_id,
...
if __name__ == "__main__":
    ...
    async def test_vectorization():
        try:
            ...
            print(f"向量化结果: 维度={result.dimension}, 耗时={result.processing_time:.3f}s")
            ...
            print(f"批量向量化: {len(batch_result.embeddings)} 个向量，耗时={batch_result.total_processing_time:.3f}s")
