...
def check_environment():
    ...
    # 检查必需的环境变量
    required_vars = ["DATABASE_URL", "REDIS_URL", "OLLAMA_BASE_URL"]
...
def start_ai_service():
    ...
    try:
        ...
        print(
            f"🌐 服务地址: http://{settings.AI_SERVICE_HOST}:{settings.AI_SERVICE_PORT}"
        )
        print(
            f"📚 API文档: http://{settings.AI_SERVICE_HOST}:{settings.AI_SERVICE_PORT}/docs"
        )
        print(
            f"📊 健康检查: http://{settings.AI_SERVICE_HOST}:{settings.AI_SERVICE_PORT}/health"
        )
        print(
            f"📈 指标监控: http://{settings.AI_SERVICE_HOST}:{settings.AI_SERVICE_PORT}/metrics"
        )
