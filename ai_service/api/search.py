...
class SearchRequest(BaseModel):
    ...
    rerank_method: RerankMethod = Field(
        default=RerankMethod.SIMILARITY,
        description="重排序方法"
    )
    ...
    date_range: Optional[Tuple[str, str]] = Field(
        default=None,
        description="日期范围过滤 (start_date, end_date)"
    )
    ...
    vector_weight: float = Field(
        default=0.7,
        description="向量搜索权重（混合搜索）",
        ge=0.0,
        le=1.0
    )
    text_weight: float = Field(
        default=0.3,
        description="文本搜索权重（混合搜索）",
        ge=0.0,
        le=1.0
    )
...
class SearchStatsResponse(BaseModel):
    ...
    search_type_stats: Dict[str, Dict[str, Any]] = Field(..., description="搜索类型统计")
...
def _convert_service_response_to_api(service_response: ServiceSearchResponse) -> SearchResponse:
    """转换服务层搜索响应到API响应格式"""
    return SearchResponse(
        query=service_response.query,
        search_type=service_response.search_type,
        results=[_convert_service_result_to_api(result) for result in service_response.results],
...
async def advanced_search(
    ...
    similarity_threshold: float = Query(default=0.5, ge=0.0, le=1.0, description="相似度阈值"),
    rerank_method: RerankMethod = Query(default=RerankMethod.RELEVANCE, description="重排序方法"),
    document_types: Optional[str] = Query(default=None, description="文档类型过滤（逗号分隔）"),
    ...
    min_score: Optional[float] = Query(default=None, ge=0.0, le=1.0, description="最小分数"),
    ...
    try:
        ...
        if document_types:
            filters.document_types = [t.strip() for t in document_types.split(",") if t.strip()]
