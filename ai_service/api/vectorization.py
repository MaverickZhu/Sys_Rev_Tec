...
class EmbeddingRequest(BaseModel):
    """嵌入请求"""
    text: str = Field(..., description="要生成嵌入的文本", min_length=1, max_length=10000)
    ...
    try:
        ...
        for doc_request in request.documents:
            try:
                # 检查文档是否已存在
                if not request.overwrite:
                    existing_vectors = await vector_db.get_document_vectors(doc_request.document_id)
                    if existing_vectors:
                        errors.append({
