...
structured_logger = StructuredLogger("api.health")
...
async def check_database_health(vector_db) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": (
                "healthy" if health_status.get("status") == "healthy" else "unhealthy"
            ),
    ...
    try:
        ...
        try:
            ...
            metrics = ServiceMetrics(
                total_requests=vectorization_stats.get("total_requests", 0)
                + search_stats.get("total_requests", 0),
        ...
        dependencies = {
            "postgresql": db_check,
            "redis": cache_check,
            "ollama": vectorization_check.get("details", {}).get(
                "ollama", {"status": "unknown"}
            ),
            "azure_openai": vectorization_check.get("details", {}).get(
        ...
        if overall_status == "healthy":
            ...
            return JSONResponse(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content=response.dict()
            )
...
async def readiness_check(
    vector_db=Depends(get_vector_db), cache_manager=Depends(get_cache_manager)
):
    """就绪检查"""
    try:
        # 检查关键依赖
...
@router.get(
    "/metrics", summary="Prometheus指标", description="获取Prometheus格式的指标数据"
)
async def prometheus_metrics(
    ...
    try:
        ...
        metrics.append(
            f"ai_service_vectorization_total {vectorization_stats.get('total_vectorizations', 0)}"
        )
        metrics.append(
            f"ai_service_search_total {search_stats.get('total_searches', 0)}"
        )
        metrics.append(
            f"ai_service_cache_hit_rate {search_stats.get('cache_hit_rate', 0.0)}"
        )
        ...
        # 响应时间指标
        metrics.append(
            f"ai_service_vectorization_response_time_seconds {vectorization_stats.get('average_response_time', 0.0)}"
        )
        metrics.append(
            f"ai_service_search_response_time_seconds {search_stats.get('average_response_time', 0.0)}"
        )

        # 错误率指标
        metrics.append(
            f"ai_service_vectorization_error_rate {vectorization_stats.get('error_rate', 0.0)}"
        )
        metrics.append(
            f"ai_service_search_error_rate {search_stats.get('error_rate', 0.0)}"
        )
...
@router.get("/version", summary="版本信息", description="获取AI服务的版本信息")
async def version_info():
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"error": f"获取版本信息失败: {str(e)}"},
        )
