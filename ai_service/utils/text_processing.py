...
class TextProcessor:
    ...
    def _chunk_by_sentences(self, text: str, max_size: int, overlap: int) -> List[TextChunk]:
        """按句子分块"""
    ...
    def _chunk_by_paragraphs(self, text: str, max_size: int, overlap: int) -> List[TextChunk]:
        """按段落分块"""
    ...
    def _chunk_sliding_window(self, text: str, window_size: int, step_size: int) -> List[TextChunk]:
        """滑动窗口分块"""
        chunks = []
        chunk_id = 0
    ...
    def _chunk_semantic(self, text: str, max_size: int, overlap: int) -> List[TextChunk]:
        """语义分块（简化版）"""
        # 这里使用句子边界的启发式方法
        # 在实际应用中，可以使用更复杂的语义分析
        ...
        for i, sentence in enumerate(sentences):
            ...
            if not should_break and current_chunk:
                ...
                topic_change_signals = [
                    '然而', '但是', '不过', '另外', '此外', '另一方面',
                    'however', 'but', 'moreover', 'furthermore', 'on the other hand'
                ]
