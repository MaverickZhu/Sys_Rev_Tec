...
class CacheManager:
    ...
    def __init__(self, redis_client: Redis):
        ...
        self.key_prefix = "ai_service"
        # 内存缓存备用方案
        self._memory_cache = {} if redis_client is None else None
        self._cache_timestamps = {} if redis_client is None else None
    ...
    async def set(
        ...
        try:
            ...
            if self.redis is None:
                ...
                self._cache_timestamps[key] = time.time() + expire_time
                return True
    ...
    async def cache_embedding(
        self, text: str, model: str, embedding: List[float], ttl: Optional[int] = None
    ) -> bool:
        """缓存文本嵌入向量"""
    ...
    async def cache_search_results(
        ...
        return await self.set(key, cache_data, ttl or settings.SEARCH_CACHE_TTL)

    async def get_cached_search_results(
        self, query: str, search_type: str, filters: Optional[Dict[str, Any]] = None
    ...
    async def cache_document_chunks(
        self, document_id: str, chunks: List[Dict[str, Any]], ttl: Optional[int] = None
    ) -> bool:
        """缓存文档分块结果"""
        ...
        return await self.set(key, cache_data, ttl or settings.CACHE_TTL)
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 按类型统计键数量
            key_stats = {"embedding": 0, "search": 0, "document_chunks": 0, "other": 0}
...
async def get_cache_manager() -> CacheManager:
    ...
    try:
        ...
        logger.warning(f"⚠️ 无法连接Redis，使用内存缓存: {e}")
...
if __name__ == "__main__":
    ...
    async def test_cache():
        try:
            ...
            cached_embedding = await cache.get_cached_embedding(
                "测试文本", "test_model"
            )
