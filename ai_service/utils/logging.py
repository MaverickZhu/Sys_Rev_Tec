...
class StructuredLogger:
    ...
    def log_vectorization(self, document_id: str, chunks: int, model: str, duration_ms: float, **kwargs):
        """记录向量化日志"""
        extra_info = " ".join(f"{k}={v}" for k, v in kwargs.items())
        self.logger.info(
    ...
    def log_search(self, query: str, search_type: str, results: int, duration_ms: float, **kwargs):
        """记录搜索日志"""
        extra_info = " ".join(f"{k}={v}" for k, v in kwargs.items())
