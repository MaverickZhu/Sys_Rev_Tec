...
structured_logger = StructuredLogger("search")
...
class IntelligentSearchService:
    ...
    def __init__(self):
        ...
        self.stats = {
            ...
            "search_type_stats": {
                search_type.value: {"count": 0, "total_time": 0.0}
    ...
    async def search(
        ...
        **kwargs,
        ...
        # 预处理查询
        processed_query = self.text_processor.clean_text(
            query, {"normalize_whitespace": True, "remove_control_chars": True}
        )
        ...
        if use_cache and self.cache_manager:
            ...
            cached_results = await self.cache_manager.get_cached_search_results(
                processed_query, search_type.value, cache_key_data
            )
        ...
        try:
            if search_type == SearchType.VECTOR:
                ...
                results = await self._semantic_search(
                    processed_query, filters, **kwargs
                )
            elif search_type == SearchType.QUESTION_ANSWER:
                results = await self._question_answer_search(
                    processed_query, filters, **kwargs
                )
            else:
            ...
            # 生成高亮和解释
            if kwargs.get(
                "enable_highlights", self.default_search_config["enable_highlights"]
            ):
                results = await self._add_highlights(processed_query, results)

            if kwargs.get(
                "enable_explanations", self.default_search_config["enable_explanations"]
            ):
            ...
            self.stats["search_type_stats"][search_type.value][
                "total_time"
            ] += processing_time
    ...
    async def _vector_search(
        ...
        similarity_threshold = kwargs.get(
            "similarity_threshold", self.default_search_config["similarity_threshold"]
        )
    ...
    async def _hybrid_search(
        ...
        for result in text_results:
            ...
            if key in combined_results:
                ...
                combined_score = (
                    existing.score * vector_weight + result.score * text_weight
                )
        ...
        max_results = kwargs.get(
            "max_results", self.default_search_config["max_results"]
        )
    ...
    async def _rerank_results(
        self, query: str, results: List[SearchResult], method: RerankMethod, **kwargs
    ) -> List[SearchResult]:
        """重排序结果"""
    ...
    async def _semantic_rerank(
        ...
        try:
            # 构建语义分析提示
            context_texts = [
                result.content for result in results[:10]
            ]  # 限制上下文长度
...
"""
            ...
            try:
                ...
                if isinstance(ranking, list) and all(
                    ...
                    # 添加未排序的结果
                    ranked_indices = {
                        rank - 1 for rank in ranking if 1 <= rank <= len(results)
                    }
    ...
    async def _relevance_rerank(
        ...
        # 简化的相关性计算
        query_keywords = set(
            self.text_processor.extract_keywords(query, max_keywords=10)
        )

        for result in results:
            content_keywords = set(
                self.text_processor.extract_keywords(result.content, max_keywords=20)
            )
    ...
    async def _diversity_rerank(
        self, results: List[SearchResult], **kwargs
    ) -> List[SearchResult]:
        """多样性重排序"""
        if len(results) <= 1:
            return results
    ...
    async def _generate_answer(
        self, query: str, context_results: List[SearchResult], **kwargs
    ) -> str:
        """生成答案"""
        if not context_results:
...
"""
    ...
    async def _call_llm(
        self, prompt: str, max_tokens: int = 200, temperature: float = 0.1
    ) -> str:
        """调用LLM"""
        # 优先使用Ollama
            ...
            for keyword in query_keywords:
                ...
                if keyword_lower in content_lower:
                    ...
                    if start_pos != -1:
                        ...
                        context_end = min(
                            len(result.content), start_pos + len(keyword) + 50
                        )
    ...
    def _calculate_text_similarity(self, query: str, content: str) -> float:
        ...
        content_keywords = set(
            self.text_processor.extract_keywords(content, max_keywords=20)
        )
...
if __name__ == "__main__":
    ...
    async def test_search():
        try:
            ...
            print(
                f"向量搜索结果: {len(response.results)} 个，耗时 {response.processing_time:.3f}s"
            )

            # 测试混合搜索
            response = await service.search(query, SearchType.HYBRID)
            print(
                f"混合搜索结果: {len(response.results)} 个，耗时 {response.processing_time:.3f}s"
            )
            ...
            if (
                response.results
                and response.results[0].metadata.get("type") == "generated_answer"
            ):
