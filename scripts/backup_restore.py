    ...
    def restore_sqlite(self, backup_file: str) -> bool:
        ...
        try:
            ...
            if os.path.exists(db_file):
                backup_existing = f"{db_file}.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def cleanup_old_backups(backup_dir: str, keep_days: int) -> int:
    ...
    try:
        ...
        for filename in os.listdir(backup_dir):
            if filename.startswith('backup_') and (filename.endswith('.sql') or filename.endswith('.gz') or filename.endswith('.json')):
...
def list_backups(backup_dir: str) -> List[Dict]:
    ...
    try:
        ...
        for filename in os.listdir(backup_dir):
            if filename.startswith('backup_') and (filename.endswith('.sql') or filename.endswith('.gz')):
                file_path = os.path.join(backup_dir, filename)
                metadata_file = file_path.replace('.sql', '.json').replace('.gz', '.json')
...
def main():
    ...
    cleanup_parser.add_argument('--keep-days', type=int, default=30, help='保留天数（默认30天）')
    ...
    if args.command == 'backup':
        ...
        deleted_count = cleanup_old_backups(args.backup_dir, args.keep_days)
        return True
