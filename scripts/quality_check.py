...
def run_command(cmd: List[str], description: str, fix_mode: bool = False, verbose: bool = False) -> Tuple[bool, str]:
    """运行命令并返回结果"""
    try:
        if verbose:
...
def check_black(fix_mode: bool = False, verbose: bool = False) -> bool:
    ...
    if fix_mode:
        cmd = ["black", "app/", "tests/", "--line-length", "88"]
        description = "Black formatting (fixing)"
    else:
        cmd = ["black", "app/", "tests/", "--check", "--diff", "--line-length", "88"]
...
def check_isort(fix_mode: bool = False, verbose: bool = False) -> bool:
    ...
    if fix_mode:
        cmd = ["isort", "app/", "tests/", "--profile", "black"]
        description = "isort import sorting (fixing)"
    else:
        cmd = ["isort", "app/", "tests/", "--check-only", "--diff", "--profile", "black"]
...
def check_ruff(fix_mode: bool = False, verbose: bool = False) -> bool:
    ...
    # 检查
    cmd_check = ["ruff", "check", "app/", "tests/"]
    if fix_mode:
    ...
    # 格式化
    if fix_mode:
        cmd_format = ["ruff", "format", "app/", "tests/"]
        success2, output2 = run_command(cmd_format, "Ruff formatting", fix_mode, verbose)
...
def check_bandit(verbose: bool = False) -> bool:
    ...
    # 先生成JSON报告
    success1, output1 = run_command(cmd, "Bandit security scan (JSON report)", False, verbose)
    
    # 再显示终端输出
    cmd_terminal = ["bandit", "-r", "app/", "-x", "tests/"]
    success2, output2 = run_command(cmd_terminal, "Bandit security scan (terminal)", False, verbose)
...
def main():
    ...
    parser.add_argument("--fast", action="store_true", help="Use Ruff instead of Flake8 (faster)")
